...
```
로컬 스킬  → .claude/skills/my-skill/    (이 프로젝트에서만 사용)
전역 스킬  → ~/.claude/skills/my-skill/  (모든 프로젝트에서 사용)
```

- **로컬 스킬**: 프로젝트 폴더 안 `.claude/skills/` — 그 폴더에서만 작동
- **전역 스킬**: 홈 폴더 `~/.claude/skills/` — 어떤 프로젝트에서도 사용 가능

> 비유: 로컬 = 특정 회사의 사내 레시피, 전역 = 어디서나 쓰는 나만의 레시피 북
