...
지금까지 배운 PPTX 테마 설정 담당을 직접 스킬로 만들어봅니다:

```markdown
---
name: pptx-maker
description: PPTX 자동 생성. "PPTX 만들어줘", "슬라이드 생성" 요청에 사용.
---

# PPTX 자동 생성 스킬 (🖥️ 통합 담당)

## 실행 순서

1. **내용 수집**: AskUserQuestion으로 슬라이드 주제와 페이지 수 확인
2. **🎨 테마 설정**: 색상 팔레트와 폰트 체계 결정
3. **✍️ 내용 작성**: 각 슬라이드의 제목과 본문 작성
4. **🖨️ 렌더링**: HTML → PPTX 변환 및 파일 출력
5. **완료 보고**: 생성된 파일 경로와 슬라이드 수 알림
```

→ 이 스킬 하나로 "/pptx-maker"만 입력하면 전체 프로세스가 자동 실행됩니다.
